...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('program-studi.store') }}" method="POST" class="p-6 space-y-6">
            @csrf

            <div>
                <label for="nama_prodi" class="block text-sm font-medium text-slate-700 mb-1">Nama Program Studi</label>
                <input type="text" name="nama_prodi" id="nama_prodi" value="{{ old('nama_prodi') }}" required class="w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('nama_prodi')
            ...
            <div>
                ...
                <input type="text" name="kepala_prodi" id="kepala_prodi" value="{{ old('kepala_prodi') }}" class="w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                @error('kepala_prodi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
