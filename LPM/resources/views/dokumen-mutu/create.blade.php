...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        <form action="{{ route('dokumen-mutu.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf

            <!-- Judul -->
            <div>
                <label for="judul" class="block text-sm font-medium text-slate-700 mb-1">Judul Dokumen <span class="text-red-500">*</span></label>
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('judul') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">
                @error('judul')
            ...
            <div>
                ...
                <input type="text" id="nomor_dokumen" name="nomor_dokumen" value="{{ old('nomor_dokumen') }}" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('nomor_dokumen') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">
                @error('nomor_dokumen')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="tahun_berlaku" name="tahun_berlaku" value="{{ old('tahun_berlaku', date('Y')) }}" required min="2000" max="2100" class="w-full rounded-lg border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm @error('tahun_berlaku') border-red-300 focus:border-red-500 focus:ring-red-500 @enderror">
                    @error('tahun_berlaku')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">Simpan Dokumen</button>
            </div>
        </form>
    </div>
</div>
